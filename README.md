...
这些第三方可执行文件不会提交到源码仓库。应用可在首次运行时下载它们；维护者也可以仅在本地放置经过校验的文件，用于构建便携版安装包。
...
当前配置生成 Windows portable 单文件应用，产物名为：

```text
release/孤帆下载器-easyuse-0.1.0-x64.exe
```

如果本地 `tools/` 中存在通过完整性校验的 `yt-dlp.exe` 和 `ffmpeg.exe`，它们会被一起打入 portable exe；否则用户可以在应用内获取或手动选择工具。安装包应通过 GitHub Releases 发布，不应提交到源码仓库。
